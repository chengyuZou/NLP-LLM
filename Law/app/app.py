...
def load_all_components(config: Config) -> Dict[str, Any]:
    """
    使用 st.cache_resource 一次性加载所有模型和索引。
    """
...
# --- 2c. 高级 RAG 逻辑 (v2) ---
# (复用 v2 pipeline 的所有辅助函数)
def simple_tokenize(text: str) -> List[str]:
    return list(jieba.cut_for_search(text))

def retrieve_bm25_topk(bm25: BM25Okapi, query: str, k: int) -> List[Tuple[int, float]]:
    try:
        q_tokens = simple_tokenize(query)
        scores = bm25.get_scores(q_tokens)
        idxs = np.argsort(scores)[::-1][:k]
        return [(index , scores[index]) for index in idxs]
    except Exception as e:
        logger.warning(f"BM25 检索失败: {e}")
        return []

def retrieve_emb_topk(vector_db: FAISS, query: str, k: int) -> List[Tuple[int, float]]:
    try:
        ...
        for doc in docs:
            ...
            if chunk_idx is not None:
                doc_list.append((int(chunk_idx), 0.0)) # RRF 只关心排名
        return doc_list
    except Exception as e:
        logger.warning(f"FAISS (Embedding) 检索失败: {e}")
        return []

def _list_to_rank_map(doc_list: List[Tuple[int, float]]) -> Dict[int, int]:
    rank_map = {}
    for rank, (idx, _) in enumerate(doc_list):
        rank_map[int(idx)] = rank
    return rank_map

def rrf_fusion(
    bm25_list: List[Tuple[int, float]], 
    emb_list: List[Tuple[int, float]], 
    k_rrf: int
) -> Dict[int, float]:
    rrf_scores = {}
    ...
    for idx in all_indices:
        score = 0.0
        if idx in bm25_rank: score += 1.0 / (k_rrf + bm25_rank[idx])
        if idx in emb_rank: score += 1.0 / (k_rrf + emb_rank[idx])
...
def multi_retrieve_and_rerank(
    ...
    """
    执行 v2 的 RRF + Rerank 检索，只返回最终排序的 Document 列表。
    """
    logger.info("执行 [高级 RAG] 检索...")
    # 1. 粗召回 (BM25 + Embedding)
    ...
    if not candidate_ids:
        logger.warning("RRF 融合后没有有效的候选文档。")
        return []
    ...
    # 5. 返回 topk_rerank 的 Document 对象
    final_docs = [doc for doc, score in sorted_candidates[:config.topk_rerank]]
    return final_docs
...
def answer_by_rag_advanced(
    ...
    """
    # 1. v2 检索
    retrieved = multi_retrieve_and_rerank(
...
if prompt := st.chat_input("请输入你的法律问题..."):
    ...
    with st.chat_message("assistant", avatar="⚖️"):
        ...
        try:
            if is_deep_rag:
                # --- 高级 RAG 流程 (v2) ---
                placeholder.markdown("🔬 **深度思考中...** (执行 RRF + Rerank)")
                ...
                # --- 标准 RAG 流程 (v1) ---
                placeholder.markdown("🔍 **标准检索中...** (执行 FAISS)")
